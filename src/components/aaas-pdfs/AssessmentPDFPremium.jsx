...
const c = {
  primary: '#b45309', // Amber 700 - Premium Gold theme
...
const styles = StyleSheet.create({
  ...
  pageHeader: { fontSize: 16, fontFamily: 'Helvetica-Bold', color: '#854d0e', marginBottom: 12, borderBottomWidth: 1.5, borderBottomColor: c.primary, paddingBottom: 5, textTransform: 'uppercase' },
  ...
  sectionTitle: { fontSize: 12, color: '#854d0e', fontFamily: 'Helvetica-Bold' },
  ...
  // Narrative
  narrativeTitle: { fontSize: 12, fontFamily: 'Helvetica-Bold', color: '#854d0e', marginBottom: 6, marginTop: 8 },
...
export default function AssessmentPDFPremium({ session, aiInsight, calculatedScores, rawScores }) {
    ...
    <Document>
      ...
      <Page size="A4" style={styles.pageOdd}>
        <View style={{ textAlign: 'center', marginBottom: 10 }}>
          <Text style={styles.rahasia}>SANGAT RAHASIA - PREMIUM PACK</Text>
          <Text style={styles.lembaga}>Lembaga Konseling dan Psikoterapi Islam</Text>
          <Image src="/logo.png" style={styles.logo} />
        </View>
        <View style={styles.hr} />
        <Text style={styles.mainTitle}>HASIL PEMETAAN KEPRIBADIAN HOLISTIK EKSKLUSIF</Text>
        ...
        <View style={styles.identityContainer}>
          ...
          <View style={styles.identityCol}>
            ...
            <IdRow label="Paket Asesmen" value="Premium Holistik" />
          </View>
        </View>
      ...
      <Page size="A4" style={styles.pageWrap} wrap>
        ...
        <Text style={styles.pageHeader}>ANALISIS HOLISTIK MENDALAM (PREMIUM)</Text>
        ...
        {aiInsight?.kekuatan_utama ? (
          <View wrap={false} style={{ marginBottom: 6 }}>
            <Text style={styles.narrativeTitle}>2. Kekuatan Utama Eksekutif</Text>
            {aiInsight.kekuatan_utama.map((k, i) => <Text key={i} style={styles.bulletItem}>• {k}</Text>)}
          </View>
        ) : null}

        <View wrap={false} style={{ marginBottom: 6 }}>
          <Text style={styles.narrativeTitle}>3. Tantangan & "Blind Spots" Tersembunyi</Text>
          ...
          <Text style={styles.narrativeBody}><Text style={{ fontFamily: 'Helvetica-Bold' }}>Karakter Internal & Etika: </Text>{aiInsight?.tantangan_dan_faktor_penghambat?.hambatan_karakter_internal || '-'}</Text>
        </View>

        <View wrap={false} style={{ marginBottom: 6 }}>
          <Text style={styles.narrativeTitle}>4. Rekomendasi Penempatan Lingkungan</Text>
          <Text style={styles.narrativeBody}><Text style={{ fontFamily: 'Helvetica-Bold' }}>Budaya Kerja Ideal: </Text>{aiInsight?.analisis_lingkungan_ideal?.ekosistem_kerja}</Text>
          <Text style={styles.narrativeBody}><Text style={{ fontFamily: 'Helvetica-Bold' }}>Motivator Psikologis Utama: </Text>{aiInsight?.analisis_lingkungan_ideal?.kebutuhan_motivasi}</Text>
        </View>

        <View wrap={false} style={{ marginBottom: 6 }}>
          <Text style={styles.narrativeTitle}>5. Panduan Pengembangan Karier</Text>
          {(aiInsight?.saran_pengembangan_spesifik || []).map((k, i) => <Text key={i} style={styles.bulletItem}>• {k}</Text>)}
        </View>
        ...
        <View style={{ marginTop: 25, padding: 15, backgroundColor: '#fcfcfc', borderRadius: 4 }}>
          <Text style={{ fontSize: 9, color: c.grey, lineHeight: 1.5, textAlign: 'center', fontStyle: 'italic' }}>
            *Laporan lengkap (Premium Pack) dirancang untuk memberikan spektrum terluas dari potensi dan titik buta (blind spots) individu, menggabungkan respon aktual DISC dan karakter mendalam HEXACO yang diinterpretasikan oleh kecerdasan buatan dengan validasi empiris konstan bertahap.
          </Text>
        </View>
